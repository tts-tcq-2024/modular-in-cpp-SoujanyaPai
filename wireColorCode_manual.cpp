...
namespace TeleComColorCoder 
{
  std::string formatColorPairsForMajorColor(MajorColor major) {
        ...
        for (int minor = 0; minor < numberOfMinorColors; ++minor) {
           WireColorPair colorPair =
              getColorFromPairNumber(static_cast<int>(major) * numberOfMinorColors + minor + 1);
          formattedPairs +=
              colorPair.toString() + " - "
              + std::to_string(getPairNumberFromColor(colorPair.getMajorColor(), colorPair.getMinorColor())) + "\n";
        }
